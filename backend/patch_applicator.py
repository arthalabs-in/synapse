...
def apply_patch(report: ResearchReport, patch: CoveragePatch, fact_ledger: FactLedger) -> PatchApplicationResult:
    ...
    known_unsupported_result_ids = {
        str(item.get("result_id", "")).strip()
        for item in fact_ledger.unsupported_claims
        if str(item.get("result_id", "")).strip()
    }

    for operation in patch.patch_operations:
        _validate_operation(operation, known_facts, known_contradictions, known_unsupported_result_ids)
...
def _validate_operation(
    ...
    known_unsupported_result_ids: set[str],
) -> None:
    ...
    if operation.result_ids:
        if operation.op not in {"remove", "remove_unsupported"}:
            raise PatchApplicationError("result_ids may only support remove operations")
        unknown_result_ids = sorted(set(operation.result_ids) - known_unsupported_result_ids)
        if unknown_result_ids:
            raise PatchApplicationError(f"unknown unsupported result_id: {', '.join(unknown_result_ids)}")

    if not operation.fact_ids and not operation.contradiction_ids and not operation.result_ids:
        raise PatchApplicationError("patch must reference valid fact_ids, contradiction_ids, or result_ids")
