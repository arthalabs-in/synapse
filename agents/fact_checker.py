    ...
    async def _classify(self, evidence_items: list[EvidenceItem], original_query: str) -> tuple[list[VerifiedFact], list[VerifiedFact], list[dict]]:
        ...
        for item in evidence_items:
            if _has_severe_quality_warning(item):
                unsupported.append({"claim": item.claim, "evidence_id": item.evidence_id, "reason": "evidence failed quality gate"})
                continue
            ...
            if judged:
                ...
                if status == "VERIFIED":
                    ...
                    unsupported.append({"claim": item.claim, "evidence_id": item.evidence_id, "reason": judged.get("explanation", "unsupported by LLM")})
                continue
                ...
                unsupported.append(
                    {
                        ...
                        "reason": "source quote conflicts with claim",
                    }
                )
