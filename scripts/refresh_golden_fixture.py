...
Phase 4.2: run this script after merging a feature that materially changes
    pipeline behavior to refresh ``tests/fixtures/demo_golden.json``.
...
Example:
    python scripts/refresh_golden_fixture.py \
        --query "What are the strongest evidence-backed approaches for building a trustworthy AI research assistant?" \
        --out tests/fixtures/demo_golden.json \
        --timeout 360
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default="tests/fixtures/demo_golden.json",
