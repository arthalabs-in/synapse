...
class Config:
    ...
    GOLDEN_RESULT_PATH: str = os.getenv("GOLDEN_RESULT_PATH", "tests/fixtures/golden_pipeline_result.json")
    ...
    # --- Phase 0.2: demo query override (Milan AI Week Gemini track pivot) ---
    DEMO_QUERY: str = os.getenv(
        "DEMO_QUERY",
        (
            "Which Gemini-first, evidence-grounded AI agent workflow should SYNAPSE build "
            "for the Milan AI Week hackathon? Focus on Gemini API or ADK function calling, "
            "grounding, multimodal inputs, evaluation, and demo feasibility."
        ),
    )
